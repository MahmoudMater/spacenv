...
import { useUpdateSpace } from "@/lib/hooks";
import { useUIStore } from "@/lib/stores";

export function EditSpaceModal() {
  const open = useUIStore((s) => s.isEditSpaceOpen);
  const draft = useUIStore((s) => s.spaceEditDraft);
  const closeEditSpace = useUIStore((s) => s.closeEditSpace);
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-1/2 left-1/2 z-51 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border border-zinc-800 bg-zinc-900 p-6 text-zinc-50 shadow-lg outline-none">
          ...
          <Dialog.Description className="mt-1 text-sm text-zinc-400">
            Update the name and description. Only you can change this space as
            the owner.
          </Dialog.Description>
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div className="space-y-2">
              <Label htmlFor="edit-space-name" className="text-zinc-200">
                Name
              </Label>
              <Input
                id="edit-space-name"
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-space-desc" className="text-zinc-200">
                Description{" "}
            ...
            <div className="flex justify-end gap-2 pt-2">
              <Button
                ...
                onClick={closeEditSpace}
              >
                Cancel
              </Button>
              <Button
                type="submit"
                disabled={updateSpace.isPending || !draft}
